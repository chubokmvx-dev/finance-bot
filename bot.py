...
CACHE = {}
CACHE_TTL = {
    "live": 90,          # live матчі — 1.5 хв
    "upcoming": 600,     # майбутні — 10 хв
    "stats": 180,        # статистика — 3 хв
    "h2h": 86400,        # h2h — 24 години (не змінюється між командами!)
    "odds": 180,         # коефіцієнти — 3 хв
    "fixture": 300,      # окремий матч — 5 хв
}
...
async def football_request(endpoint, params={}, cache_type="stats"):
    ...
    logger.info(f"API request #{REQUEST_COUNT['today']} today: {endpoint}")
        ...
        async with aiohttp.ClientSession() as s:
            async with s.get(
                ...
                if r.status == 200:
                    ...
                    logger.info(f"Requests remaining today: {remaining}")
                    ...
                    logger.warning("Rate limit hit! Daily limit exceeded.")
                    return None
...
def calc_ht_probability(h2h_data):
    if not h2h_data or len(h2h_data) < 2:
        return 45, ["📋 Недостатньо H2H даних"]
    score = 0
    factors = []
    ht_goals = []
    ft_goals = []
    ...
    for fix in h2h_data[:8]:
        ...
        h_ht = ht.get("home")
        a_ht = ht.get("away")
        h_ft = g.get("home") or 0
        a_ft = g.get("away") or 0
        ft_goals.append(h_ft + a_ft)
        if h_ht is not None and a_ht is not None:
            ht_goals.append((h_ht or 0) + (a_ht or 0))

    # Якщо є дані 1Т — використовуємо їх
    if len(ht_goals) >= 2:
        ...
        if avg >= 2.0:
            score += 35; factors.append(f"⚽ H2H 1Т: {avg:.1f} голів")
        elif avg >= 1.5:
        ...
        if over05 >= n * 0.8:
            score += 30; factors.append(f"✅ {over05}/{n} матчів — гол в 1Т")
        elif over05 >= n * 0.6:
            score += 18; factors.append(f"🟡 {over05}/{n} матчів — гол в 1Т")
        else:
            score += 5; factors.append(f"⚠️ {over05}/{n} матчів — гол в 1Т")
    ...
    # Fallback — рахуємо по загальних голах (40% падає в 1Т)
    elif ft_goals:
        avg_ft = sum(ft_goals) / len(ft_goals)
        n = len(ft_goals)
        avg_ht_est = avg_ft * 0.4
        over25_ft = sum(1 for g in ft_goals if g >= 3)

        factors.append(f"📋 H2H: {avg_ft:.1f} голів за матч (оцінка 1Т)")
        if avg_ft >= 3.5:
            score += 65; factors.append("🔥 Дуже результативні матчі")
        elif avg_ft >= 2.5:
            score += 50; factors.append("✅ Результативні матчі")
        elif avg_ft >= 1.5:
            score += 35; factors.append("🟡 Помірна результативність")
        else:
            score += 15; factors.append("⚠️ Мало голів в H2H")

        if over25_ft >= n * 0.6:
            score += 20; factors.append(f"✅ {over25_ft}/{n} матчів — 3+ голів")
...
async def show_top_bets(update, context):
    ...
    # ── LIVE матчі (максимум 5) ──
    live = await get_live_matches()
    if live:
        await msg.edit_text(f"⭐ Аналізую live ({min(5,len(live))} матчів)...")
        for fix in live[:5]:
            m = parse_fixture(fix)
            if not m: continue
            # Спочатку тільки H2H (1 запит) — якщо є сигнал, беремо статистику
            h2h = await get_h2h(m["home_id"], m["away_id"]) if m.get("home_id") else []
            prob_quick, _ = calc_goal_probability(m, {}, h2h)
            if prob_quick >= 40:  # тільки перспективні матчі
    ...
    if upcoming:
        upcoming = sorted(upcoming, key=lambda x: x.get("fixture",{}).get("date",""))[:5]
        for fix in upcoming:
            m = parse_fixture(fix)
            if not m: continue
            h2h = await get_h2h(m["home_id"], m["away_id"]) if m.get("home_id") else []
            ...
            strong = [r for r in recs if r["confidence"] >= 58]
            if strong:
                try:
                    dt = datetime.fromisoformat(m["date"].replace("Z","+00:00"))
                    ...
                    m["kick_off"] = "??:??"
                top.append({"m": m, "recs": strong[:3], "prob": prob, "type": "pre"})
            await asyncio.sleep(0.2)
