...
def recommend_totals(match, prob_score, is_live=True):
...
def format_analysis(match, stats, recs, factors, is_live):
    ...
    recs_text = "💡 *Рекомендації по тоталах:*\n\n"
    if recs:
...
async def show_top_bets(update, context):
    ...
    if not live:
        await msg.edit_text("😐 Немає live матчів для аналізу"); return
    ...
    for fix in live[:10]:
        ...
        if strong:
            top.append({"m": m, "recs": strong[:2], "prob": prob})
        count += 1
        await asyncio.sleep(0.3)

    if not top:
        await msg.edit_text("😐 Зараз немає ставок >60%\n\nСпробуйте пізніше — ситуація змінюється кожні хвилини!"); return

    top.sort(key=lambda x: -x["prob"])
    text = "⭐ *Топ ставки (від 60%):*\n\n"
    for item in top[:6]:
        m = item["m"]
        text += f"⚽ *{m['home']} {m['home_goals']}:{m['away_goals']} {m['away']}* | {m['elapsed']}'\n"
    ...
    btns = [[InlineKeyboardButton(f"📊 {i['m']['home'][:10]} vs {i['m']['away'][:10]}", callback_data=f"a_{i['m']['id']}_1")] for i in top[:5]]
    await msg.edit_text(text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(btns))
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    recs = recommend_totals(match, prob, is_live)
