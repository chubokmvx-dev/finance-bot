...
def fmt(n): return f"{n:,.0f} грн".replace(",", " ")
...
async def process_ai(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    ...
    prompt = (
        ...
        f"Поверни ТІЛЬКИ валідний JSON без markdown:\n"
        f'{{"type":"expense або income","amount":число,"currency":"UAH або USDT або USD","amount_uah":сума_в_гривнях,"category":"catId","income_type":"typeId","desc":"опис","message":"підтвердження"}}\n'
    ...
    try:
        ...
        await msg.edit_text("❌ Не вдалось розпізнати. Спробуйте ще раз")
        return
    if "error" in parsed:
    ...
    if parsed["type"] == "expense":
        ...
        cs = f" ({fmtd(entry['amount'])} x {fmt(rate)})" if entry["currency"] != "UAH" else ""
    ...
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get("https://api.monobank.ua/personal/client-info",
                ...
                if r.status != 200:
                    await update.callback_query.edit_message_text("❌ Невірний токен")
                    return
                info = await r.json()
        ...
        if m not in u["months"]:
            u["months"][m] = {"income": [], "expenses": []}
        existing = set(e.get("mono_id") for e in u["months"][m].get("expenses",[]) + u["months"][m].get("income",[]) if e.get("mono_id"))
        ...
        for tx in transactions:
            ...
            desc = tx.get("description") or tx.get("comment") or "Mono"
            mcc = tx.get("mcc", 0)
            cat = "other"
            for cat_id, mccs in mcc_map.items():
                if mcc in mccs:
                    cat = cat_id
                    break
            entry = {"id": int(datetime.now().timestamp()*1000)+count, "mono_id": tx["id"],
                     "amount": amount, "currency": "UAH", "amount_uah": amount,
                     "desc": desc, "date": datetime.fromtimestamp(tx["time"]).isoformat(), "source": "monobank"}
            if tx["amount"] < 0:
                entry["category"] = cat
                u["months"][m]["expenses"].append(entry)
            else:
        ...
        await update.callback_query.edit_message_text(
            "✅ Все актуально!" if count == 0 else f"✅ Додано {count} транзакцій 🏦")
    except Exception as e:
...
async def run_analysis(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    prompt = (f"Ти дружній фінансовий аналітик. Відповідай українською.\n"
              f"Дані за {ml(mk())}: Дохід: {fmt(inc)}, Витрати: {fmt(exp)}, Баланс: {fmt(inc-exp)}\n"
              f"Накопичено: {fmt(sav)} ({pct}%), Ціль: {fmt(tgt)}\nПо категоріям: {cat_str}\n\n"
              f"Зроби аналіз:\n📊 Загальна картина — 2 речення\n"
              f"🔴 На чому зекономити — 2-3 пункти з цифрами\n"
              f"✅ Що добре — 1-2 пункти\n💡 Порада на сьогодні — одна дія")
...
async def show_invest(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    bal = max(0, inc-exp)
    rate = await get_rate()
    prompt = (f"Ти фінансовий консультант. Відповідай українською. Попередь про ризики.\n"
              f"Вільний капітал: {fmt(bal)} ({fmtd(bal/rate)}), Дохід: {fmt(inc)}\n"
              f"Дай 4 варіанти: ОВДП, депозит, ETF, стейкінг USDT на Binance.\n"
              f"Для кожного: дохідність за місяць, конкретна сума, головний ризик.")
    res = await ask_claude(prompt, 800)
    if not res:
        await msg.edit_text("❌ AI недоступний")
        return
    await msg.edit_text(f"💡 Куди вкласти\n\n⚠️ Загальна інформація, не фінансова порада.\n\n💳 Капітал: {fmt(bal)}\n\n{res}")
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    d = q.data
    uid = update.effective_user.id
    if d == "dashboard": await send_dashboard(update, context)
    elif d == "analysis": await run_analysis(update, context)
    elif d == "sync_mono": await do_sync_mono(update, context)
    elif d == "enter_mono":
        ...
        await q.edit_message_text("🎯 Введіть суму цілі (грн):\n\nНаприклад: 5000")
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "➕ Додати":
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("📉 Витрата", callback_data="add_exp"), InlineKeyboardButton("📈 Дохід", callback_data="add_inc")]])
